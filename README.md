# Система логування подій

Навчальний груповий проєкт з курсу «Конструювання програмного забезпечення». Застосунок з вебінтерфейсом, який дозволяє створювати, переглядати, фільтрувати та сортувати події (логи)

## Автори проєкту

- Гринишин Вікторія
- Бащак Андрій
- Світлик Дмитро
- Нікітіна Ксенія
- Погорельцева Анна
- Таркані Мирослава

## Основні можливості

- **Create log** – створення нового запису з полями:
  - `Date`
  - `Device ID`
  - `Category`
  - `Description`
- **Фільтрація за датою** – поля `From` і `To` дозволяють обрати часовий діапазон
- **Пошук за Device ID** – знаходить усі логи, що містять введений `Device ID`
- **Пошук за категорією** – фільтрація за полем `Category`
- **Пошук за частиною опису** – поле `Description` дозволяє знайти всі записи, де в описі міститься введене слово чи фраза
- **Сортування логів** за допомогою `Sort by`, де доступні варіанти:
  - `ID`
  - `Event date`
  - `Device ID`
  - `Category`
- **Напрям сортування** – вибір у полі `Order`:
  - `Ascending` (за зростанням)
  - `Descending` (за спаданням)
- **Search** – запуск пошуку за всіма заповненими фільтрами
- **Clear filters** – очищення всіх фільтрів одним кліком


## Шляхи проєкту

`/home` - домашня сторінка сайту

`/dashboard` - інтерфейс користувача

`/register` - сторінка реєстрації

`/login`   - сторінка авторизації
    
Якщо Ви спробуєте перейти на /dashboard без авторизації, Вас перекине на /login.
При введені просто /, будучи авторизованими, то вас перекине на /home, але ви можете одразу перейти на /dashboard без повторної авторизації.

## Як запустити проєкт

Перед початком роботи потрібно відкрити термінал у порожній папці, в якій ви хочете зберегти проєкт, та виконати наступні кроки:


1. Клонувати репозиторій:
```bash
git clone https://github.com/hrynyshynukd/team_project.git
```
```bash
cd team_project
```

2. Додати у корінь проєкту файл з назвою .env та власним ключем (приклад):
```bash
.env
```
```bash
JWT_SECRET=your_key
```

3. Встановити залежності:
```bash
npm install
```

4. Запустити застосунок:
```bash
npm start
```

5. Відкрити браузер і перейти за посиланням:
```bash
http://localhost:3000
```
